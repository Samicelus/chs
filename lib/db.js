...
async function connect({_id, dbType, connectStr, dbName, login, password}) {
    switch (dbType) {
        case 'mongodb':
            let url = `mongodb://${login?(login+':'+password+'@'):''}${connectStr}${dbName?'/'+dbName:''}`;
            pool.mongodb[_id] = await mongoose.createConnection(url, {useNewUrlParser: true});
            break;
        case 'mysql':
            let mysqlConfig = {
...
// 获取连接
async function getConnect({_id, dbType, connectStr, dbName, login, password}) {
    switch (dbType) {
        case 'mongodb':
 ...
 * 显示所有表
 */
exports.showTable = async function ({_id, dbType, connectStr, dbName, login, password}) {
    let tables;
    let conn = await getConnect({_id, dbType, connectStr, dbName, login, password});
    switch (dbType) {
        case 'mongodb':
 ...
 * 显示表结构
 */
exports.showModel = async function ({_id, dbType, connectStr, dbName, login, password, collectionName}) {
    let structure;
    let conn = await getConnect({_id, dbType, connectStr, dbName, login, password});
    switch (dbType) {
        case 'mongodb':
